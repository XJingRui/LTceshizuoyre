...
def run_training():
    ...
    model_save_path = os.path.join(model_dir, f"best_model_lstm_{business_module}_{target_vin}.pth")
    history_save_path = os.path.join(history_dir, f"history_lstm_{business_module}_{target_vin}.json")

    print(f"\n🔥 岚图 LSTM 预测引擎启动 | 训练设备: {config.device} | 批次大小: {config.batch_size}")
