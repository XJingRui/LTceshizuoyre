...
from model_bilstm import BiLSTMPredictor
...
def run_evaluation():
    ...
    model_path = os.path.join("models", f"best_model_bilstm_{business_module}_{target_vin}.pth")
    test_results_dir = "test_results_bilstm"
    os.makedirs(test_results_dir, exist_ok=True)

    print(f"\n🔍 启动 BiLSTM 模型测试评估引擎 | 车辆: {target_vin} | 业务: {business_module}")
    print("-" * 70)

    if not os.path.exists(model_path):
        raise FileNotFoundError(f"❌ 找不到训练好的模型权重: {model_path}，请先运行 train_bilstm.py！")
    ...
    model.eval()
    print("✅ BiLSTM 模型权重加载成功！开始进行全量推理...")
    ...
    def draw_comparison_plot(time_steps, true_data, pred_data, title_suffix, filename_suffix):
        ...
        for i, target_name in enumerate(target_names):
            ...
            ax.plot(time_steps, pred_data[:, i], label='BiLSTM 预测 (Prediction)', color='blue', linewidth=1.5,
                    linestyle='--', alpha=0.9)

            unit = "kW" if "功率" in target_name else "℃"
            ax.set_ylabel(f"{target_name} ({unit})", fontsize=12)
            ax.set_title(f"[BiLSTM] [{title_suffix}] 预测对比曲线: {target_name} (VIN: {target_vin})", fontsize=14)
